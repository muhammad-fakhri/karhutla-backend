...
function HakAksesPage(props) {
  ...
  const { data: dataNonPatroli } = useSWR(
    isAuthenticated ? "/non_patroli/list" : null,
    UserService.getNonPatroliUsers
  );
  const { data: dataPatroliNonLogin } = useSWR(
    isAuthenticated ? "/non_login/list" : null,
    UserService.getPatroliNonLoginUsers
  );
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    if (dataPatroliNonLogin && isAuthenticated)
      ...
      <Grid container justify="center" className={classes.gridContainer}>
        ...
        <Grid item xs={10} align="center" className={classes.gridItem}>
          <AppBar position="static" color="default">
            ...
            >
              <Tab label="Personil Manggala Agni" wrapped {...a11yProps(0)} />
              <Tab label="Daops" {...a11yProps(1)} />
              <Tab label="Balai/Pusat" {...a11yProps(2)} />
              <Tab label="Patroli Non Login" {...a11yProps(3)} />
            </Tabs>
          </AppBar>
              ...
              editable={{
                onRowUpdate: (newData, oldData) =>
                  new Promise((resolve, reject) => {
                    UserService.updateNonPatroliUser(newData).then((result) => {
                      if (result.success) {
                        if (oldData) {
                        ...
                        showAlert(true, "Update hak akses daops berhasil");
                        resolve();
                      } else {
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    UserService.deleteNonPatroliUser(oldData).then((result) => {
                      if (result.success) {
                        const dataDelete = [...daopsState];
                        const index = oldData.tableData.id;
                        dataDelete.splice(index, 1);
                        setDaopsState(dataDelete);
                        showAlert(true, "Hapus hak akses daops berhasil");
                        resolve();
                      } else {
                        showAlert(false, result.message[0]);
                        reject();
                  ...
                  new Promise((resolve, reject) => {
                    UserService.deleteNonPatroliUser(oldData).then((result) => {
                      if (result.success) {
                        ...
                        showAlert(true, "Hapus hak akses balai/pusat berhasil");
                        resolve();
                      } else {
                        showAlert(false, result.message[0]);
                        reject();
          ...
          <TabPanel value={value} index={3}>
            <MaterialTable
              ...
              editable={{
                onRowAdd: (newData) =>
                  new Promise((resolve, reject) => {
                    UserService.addPatroliNonLoginUser(newData).then(
                      (result) => {
                        if (result.success) {
                          setPatroliNonLogin([...patroliNonLogin, newData]);
                          showAlert(
                            true,
                ...
                onRowUpdate: (newData, oldData) =>
                  new Promise((resolve, reject) => {
                    UserService.updatePatroliNonLoginUser(newData).then(
                      (result) => {
                        if (result.success) {
                          if (oldData) {
                            setPatroliNonLogin((prevState) => {
                              const data = [...prevState];
                              data[data.indexOf(oldData)] = newData;
                              return data;
                            });
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    UserService.deletePatroliNonLoginUser(oldData).then(
                      (result) => {
                        if (result.success) {
                          const dataDelete = [...patroliNonLogin];
                          const index = oldData.tableData.id;
                          dataDelete.splice(index, 1);
                          setPatroliNonLogin(dataDelete);
                          showAlert(
                            true,
