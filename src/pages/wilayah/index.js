            ...
            editable={{
              onRowAdd: (newData) =>
                new Promise((resolve, reject) => {
                  WilayahService.addWilayah(newData).then(async (result) => {
                    if (result.success) {
                      let data = await WilayahService.getAllWilayah();
                      setValues({
                        ...values,
                        wilayah: data,
                        alertMessage: "Tambah Wilayah Berhasil",
                        successAlert: true,
                      });
                      resolve();
                    } else {
                      setValues({
                        ...values,
                        alertMessage:
                          "Tambah Wilayah Gagal, " + result.message[0],
                        successAlert: false,
                      });
                      reject();
                    }
              ...
              onRowUpdate: (newData, oldData) =>
                new Promise((resolve, reject) => {
                  WilayahService.updateWilayah(newData, oldData).then(
                    (result) => {
                      if (result.success) {
                        ...
                        setValues({
                          wilayah: [...dataUpdate],
                          alertMessage: "Update Wilayah Berhasil",
                          successAlert: true,
                        });
                        resolve();
                      } else {
                        setValues({
                          ...values,
                          alertMessage:
                            "Update Wilayah Gagal, " + result.message[0],
                          successAlert: false,
                        });
                        reject();
                      }
              ...
              onRowDelete: (oldData) =>
                new Promise((resolve, reject) => {
                  WilayahService.deleteWilayah(oldData).then((result) => {
                    if (result.success) {
                      const dataDelete = [...values.wilayah];
                      const index = oldData.tableData.id;
                      dataDelete.splice(index, 1);
                      setValues({
                        ...values,
                        wilayah: [...dataDelete],
                        alertMessage: "Hapus Wilayah Berhasil",
                        successAlert: true,
                      });
                    } else {
                      setValues({
                        ...values,
                        alertMessage:
                          "Hapus Wilayah Gagal, " + result.message[0],
                        successAlert: false,
                      });
                    }
                    showAlert();
